...
{@fig-combo} gives a quick visual summary of what we'll do in the code that follows.
...
![Data frame with comma-delimited strings in artist column transformed to tidy output with `separate_longer_delim()`.](song_artist_longer_transform_separate_longer-min.png){#fig-separate-longer fig-alt="A data frame with two character columns, song and artist, that contains multiple artists in the artist column as a string separated by commas is transformed into a tidy data frame with one song per row and one artist per row using separate_longer_delim()."}
